...
フル実行では、対象 CIDR、最大ホスト数、外部通信、Cloudflare 速度測定の上限（下り約 180 MB + 上り約 20 MB）、通信品質の監視時間（60秒）を開始前にまとめて表示します。レポートの「モニタ」では、遅延・損失・ジッター・瞬断、下りと上りの実効速度、過去の診断値の推移をまとめて確認できます。
