...
- 引数なしの基本診断は、PC・NIC・既定ゲートウェイまでを確認します。LAN 全探索と外部サービス通信は行いません。
...
3. 最初は `[1] 基本診断` を実行します。
4. 構成図が必要なら `[2] 詳細LAN調査` を選び、表示された対象範囲が自分の管理する LAN であることを確認します。
...
|---|---|
| `1` | ローカル限定の基本診断 |
| `2` | 確認付きの詳細 LAN 調査と HTML レポート生成 |
| `V` | Cloudflare を使う速度測定。下り約 180 MB + 上り約 20 MB が上限 |
| `3` | 既定ゲートウェイの遅延・瞬断モニタ。外部監視は別途確認 |
| `F` | PC 側の修復候補をプレビュー、適用、またはロールバック |
| `S` | 診断または監視の定期実行を設定 |
| `O` | 前回の通常レポートを開く |
...
`[P]` の公開用レポートは、IP・MAC・名称などを一貫した仮名へ置き換え、履歴、プロセス、接続先、外部回線情報などを除外します。これは「匿名化の保証」ではありません。珍しい構成、測定時刻、端末数、自由記述などから推測される可能性があるため、公開前に必ず目視確認してください。
